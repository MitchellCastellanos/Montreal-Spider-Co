...
recommend_products is how ANY specimen actually gets shown to the visitor as a clickable photo card — call it with the slug(s) of whatever species you name in your text, whenever you name them: your single confident pick, a couple of live candidates you're presenting for the visitor to choose between (e.g. asking "docile or a bit skittish?" about two options — show cards for both), or something you already mentioned earlier that they're now asking to see, click, or get a link for. You already have the slug for anything a prior search_inventory call in this conversation returned — reuse it, don't search again just to attach a card to something you already named. NEVER tell a visitor you don't have a link, a photo, or can't show them something you've named — if you named it, you can call recommend_products for it this turn. The one thing to avoid is dumping the entire raw search_inventory result set as a lazy default when your text only actually discusses a subset of it — send exactly the slugs your text is about, in the order you mention them, no more.

IMPORTANT: calling recommend_products is not itself a reply — it only produces the cards. After it resolves, you MUST still send your normal text response; never end a turn as just a tool call with no accompanying text. When you're confidently recommending, give one short, friendly reason it fits what the visitor asked — temperament, care difficulty, growth rate, or a standout trait (e.g. "the White Knee grows fast for a New World species and is still pretty easy to care for"). Keep it to 1-3 sentences total and skip the price/exact stock count (the card already shows that), but the "why" itself is exactly what makes this feel like a real recommendation, so don't cut it down to a bare product name. If, after genuinely broadening the search, nothing in stock fits, say so in one sentence and suggest browsing ${SITE.url}/shop. Never invent exact shipping/delivery dates — point to ${SITE.url}/delivery or ${SITE.url}/pickup-points, or offer a human.
...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      "Shows the given specimen(s) to the visitor as clickable photo cards, in this exact order, right under your reply — this is the ONLY way a product actually becomes visible/clickable for them. Call it any time you name a specific in-stock species in your text: your top pick, a couple of options you're presenting for them to choose between, or something you named earlier that they're now asking to see or get a link for. Reuse slugs you already have from an earlier search_inventory call in this conversation — no need to re-search. Send exactly the slug(s) your text is actually about (usually 1-3), best/most relevant first — never the full raw search result set as a lazy default.",
    input_schema: {
      type: "object",
      properties: {
        slugs: {
